...
async def gen_simile(prompt: Prompt):
    ...
    '''
    completion = llm.create_completion(pre_prompt + prompt.prompt, max_tokens=512, temperature=0.5)
...
async def gen_critique(prompt: Prompt):
    ...
    '''
    completion = llm.create_completion(pre_prompt + prompt.prompt, max_tokens=4096, temperature=0.5)
    return { 'prompt': prompt.prompt, 'completion': completion['choices'][0] }
